...
function GMGenie.Macros.onLoad()
    ...
    UnitPopupButtons["GMGenie_Silence"] = { text = "Silence", dist = 0 };
    table.insert(UnitPopupMenus["GMGenie_Commands"], "GMGenie_Silence");
	...
	UnitPopupButtons["GMGenie_Group_List"] = { text = "Group List", dist = 0 };
    table.insert(UnitPopupMenus["GMGenie_Commands"], "GMGenie_Group_List");
	UnitPopupButtons["GMGenie_Kick"] = { text = "Kick", dist = 0 };
    table.insert(UnitPopupMenus["GMGenie_Commands"], "GMGenie_Kick");
    ...
    for _, button in ipairs(GMGenie.Macros.menuItems) do
        ...
        local teamPopupMenu = UnitPopupMenus["TEAM"]
        if teamPopupMenu then
          table.insert(teamPopupMenu, #teamPopupMenu - 1, "GMGenie_" .. button);
        end
...
function GMGenie.Macros.contextMenuClick(self)
    ...
    if (startPos == 1) then
        ...
        if button == "Silence" then
            GMGenie.Macros.silence(name);
		...
		elseif button == "Group_List" then
            GMGenie.Macros.grouplist(name);
		elseif button == "Kick" then
            GMGenie.Macros.kick(name);
        else
...
function GMGenie.Macros.kick(name)
    SendChatMessage(".kick " .. name, "GUILD");
end

function GMGenie.Macros.grouplist(name)
    GMGenie.showGMMessage("Group list for: " .. name);
	SendChatMessage(".group list " .. name, "GUILD");
...
function GMGenie.Macros.silence(name)
    SendChatMessage(".mute " .. name .. " 1h (Silenced)", "GUILD");
end
