...
function GMGenie.Spawns.move(x, y, z, o)
    ...
    forwardBackward = tonumber(forwardBackward);
    ...
    if not (forwardBackward and leftRight and upDown and rotate and x and y and z and o) then
        GMGenie.Spawns.waitingForGps = 0;
        GMGenie.showGMMessage("Invalid spawn movement amount or coordinates.");
        return false;
    end

    forwardBackward = forwardBackward * GMGenie.Spawns.direction.forwardBackward;
    leftRight = leftRight * GMGenie.Spawns.direction.leftRight;
    o = deg(o);
...
function GMGenie.Spawns.object(objectId)
	...
	if not objectId then
		GMGenie.showGMMessage("Invalid game object ID.");
		return false;
	end
...
function GMGenie.Spawns.npc(npcId)
	...
	if not npcId then
		GMGenie.showGMMessage("Invalid NPC ID.");
		return false;
	end
